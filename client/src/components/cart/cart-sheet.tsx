  ...
  const createOrderMutation = useMutation({
    mutationFn: async () => {
      ...
      const orderPromises = carts.map(async (cart) => {
        const orderTotal = getCartTotal(cart.distributorId);
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Seus pedidos foram enviados para processamento."
      });
    },
    onError: (error: Error) => {
      toast({
      ...
      <SheetContent className="flex flex-col h-full" side="right">
        ...
        {carts.length > 0 && (
          <div className="border-t pt-4 mt-auto space-y-4">
            ...
            >
              {createOrderMutation.isPending ? "Processando..." : "Finalizar Pedidos"}
            </Button>
          </div>
        )}
      </SheetContent>
